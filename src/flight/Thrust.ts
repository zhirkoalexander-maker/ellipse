...
import { difficultyMultiplier } from '../config/settings';
...
export function applyThrust(state: FlightState, dt: number, direction?: [number, number, number]): void {
  ...
  const diffMul = difficultyMultiplier(loadSettings().difficulty);
  const forceN = engine.thrust * 1000 * state.throttle * diffMul;
