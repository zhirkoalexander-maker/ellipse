...
import { mapBasis, mapTrajectory, maneuverVector, projectMap } from '../flight/MapNavigation';
import './OrbitMap.css';
...
export class OrbitMap {
 ...
 private plane:'orbit'|'xz'|'xy'='orbit';
 private zoom=1;
 ...
 constructor(private snapshot:()=>MapSnapshot, private burn:(dv:Vec3)=>string, private cancel:()=>void, private fly:(target:string)=>boolean, private opened:()=>void = ()=>{}) {
  ...
  <nav><button data-map="orbit">Orbit</button><button data-map="target">Target</button><button data-map="system">System</button><button data-map="reset" title="Centre view">⌖</button></nav>
  ...
  <hr><strong>Course correction</strong><small>Preview assumes a short burn from here.</small>
  ...
  <div class="map-burn-actions"><button data-map="clear">Clear</button><button data-map="burn">Execute burn</button><button data-map="stop">Stop</button></div>
  <output id="map-status" aria-live="polite">Launch to adjust your orbit.</output></aside>
  <footer><span><i class="map-current"></i>Current orbit <i class="map-preview"></i>After burn</span><label>View <select id="map-plane"><option value="orbit">Orbital plane</option><option value="xz">Top</option><option value="xy">Side</option></select></label><button data-map="minus" aria-label="Zoom out">−</button><button data-map="plus" aria-label="Zoom in">+</button></footer>`;
  ...
  this.life.listen(ui,'click',e=>{
   ...
   if(a==='close')this.close();
   ...
   else if(a==='clear'){this.dv=[0,0,0];this.root.querySelectorAll<HTMLInputElement>('input').forEach(i=>i.value='0');}
    ...
    this.status(this.burn(maneuverVector(r,v,this.dv)),true);
 ...
 private fit(mode:typeof this.mode){this.mode=mode;this.plane=mode==='orbit'?'orbit':'xz';(this.root.querySelector('#map-plane') as HTMLSelectElement).value=this.plane;this.pan=[0,0];this.zoom=1;}
 ...
 private draw=(time:number)=>{
  ...
  const basis=mapBasis(r,v,this.plane), origin:Vec3=[...ref.position];
  ...
  const delta=maneuverVector(r,v,this.dv);
  if(new Vector3(...delta).length()>.01)path(mapTrajectory(r,v.map((x,i)=>x+delta[i]!) as Vec3,ref.mass,ref.radius||1),ref.position,'#f4bb55',true);
  this.hits=[];
  for(const b of s.bodies as MapBody[]){if(this.mode==='orbit'&&b!==ref)continue;const [x,y]=project(b.position);if(x<-80||y<-80||x>w+80||y>h+80)continue;const radius=Math.max(3,Math.min(2000,(b.radius||1)/scale));
