  ...
  private updateInner(_dt: number): void {
    ...
    if (!this.grounded) {
      ...
      if (nearestBody && bodyR > 0 && isFinite(nearestDist)) {
        ...
        } else if (d < surfaceR + 250 && isFinite(vertSpeed) && Math.abs(vertSpeed) > 50) {
    ...
    if (!this.crashed) {
      ...
      if (!this._debugShown) {
        ...
        dbg.id = 'rocket-debug';
        dbg.innerHTML = `v0.67<br>C=freecam F=reset T=SAS W=throttle<br>rocketBottomY=${this.rocketBottomY.toFixed(4)}<br>visualOffset=${visualOffset.toFixed(4)}<br>VS=${VISUAL_SCALE.toExponential(2)}<br>RVS=${ROCKET_VISUAL_SCALE}`;
